package com.briangleason.darknessdraft;

import net.minecraft.client.Minecraft;
import net.minecraft.client.multiplayer.ClientLevel;
import net.minecraft.client.renderer.GameRenderer;
import net.minecraft.resources.ResourceKey;
import net.minecraft.world.effect.MobEffects;
import net.minecraft.world.level.Level;
import net.minecraft.world.level.dimension.DimensionType;
import net.minecraft.util.Mth;


public class DarknessClientUtils {

    // assuming lighting applied to all dimensions equally

    private static boolean isDark(Level level) {
        ResourceKey<Level> dimension = level.dimension();
        DimensionType dimensionType = level.dimensionType();
        if (dimension == Level.OVERWORLD) {
            return DarknessDraft.Config.client_darkOverworld;
        } else if (dimension == Level.NETHER) {
            return DarknessDraft.Config.client_darkNether;
        } else if (dimension == Level.END) {
            return DarknessDraft.Config.client_darkEnd;
        } else if (dimensionType.hasSkyLight()) {
            return DarknessDraft.Config.client_darkDefault;
        } else {
            return DarknessDraft.Config.client_darkSkyless;
        }
    }


    private static float skyFactor(ClientLevel level) {
        if (!DarknessDraft.Config.client_blockLightOnly && isDark(level)) {
            if (level.dimensionType().hasSkyLight()) {
                final float angle = level.getTimeOfDay(0);
                if (angle > 0.25f && angle < 0.75f) {
                    final float oldWeight = Math.max(0, (Math.abs(angle - 0.5f) - 0.2f)) * 20;
                    final float moon = DarknessDraft.Config.client_ignoreMoonPhase ? 0 : level.getMoonBrightness();
                    final float moonInterpolated = (float) Mth.lerp(moon, DarknessDraft.Config.client_minimumMoonLevel, DarknessDraft.Config.client_maximumMoonLevel);
                    return Mth.lerp(oldWeight * oldWeight * oldWeight, moonInterpolated, 1f) ;
                } else {
                    return 1;
                }
            } else {
                return 0;
            }
        } else {
            return 1;
        }
    }

    public static boolean enabled = false;
    private static final float[][] LUMINANCE = new float[16][16];

    public static int darken(int c, int blockIndex, int skyIndex) {
        final float lTarget = LUMINANCE[blockIndex][skyIndex];
        final float r = (c & 0xFF) / 255f;
        final float g = ((c >> 8) & 0xFF) / 255f;
        final float b = ((c >> 16) & 0xFF) / 255f;
        final float l = luminance(r, g, b);
        final float f = l > 0 ? Math.min(1, lTarget / l) : 0;

        return f == 1f ? c : 0xFF000000 | Math.round(f * r * 255) | (Math.round(f * g * 255) << 8) | (Math.round(f * b * 255) << 16);
    }

    public static float luminance(float r, float g, float b) {
        return r * 0.2126f + g * 0.7152f + b * 0.0722f;
    }

    public static void updateLuminance(float tickDelta, Minecraft client, GameRenderer worldRenderer, float prevFlicker) {
        final ClientLevel ClientLevel = client.level;

        if (ClientLevel != null) {

            if (!isDark(ClientLevel) || client.player.hasEffect(MobEffects.NIGHT_VISION) ||
                    (client.player.hasEffect(MobEffects.CONDUIT_POWER) && client.player.getWaterVision() > 0) || ClientLevel.getSkyFlashTime() > 0) {
                enabled = false;
                return;
            } else {
                enabled = true;
            }
            enabled = true;

            // ignoring skyfactor for now
            final float dimSkyFactor = skyFactor(client.level);
            final float ambient = client.level.getSkyDarken(1.0F);
            final DimensionType dim = client.level.dimensionType();
            final boolean blockAmbient = !isDark(ClientLevel);

            for (int skyIndex = 0; skyIndex < 16; ++skyIndex) {
                float skyFactor = 1f - skyIndex / 15f;
                skyFactor = 1 - skyFactor * skyFactor * skyFactor * skyFactor;
                skyFactor *= dimSkyFactor;

                float min = Math.max(skyFactor * 0.05f, (float) DarknessDraft.Config.client_minimumLight);
                final float rawAmbient = ambient * skyFactor;
                final float minAmbient = rawAmbient * (1 - min) + min;
                final float skyBase = dim.brightness(skyIndex) * minAmbient;

                min = Math.max(0.35f * skyFactor, (float) DarknessDraft.Config.client_minimumLight);
                float v = skyBase * (rawAmbient * (1 - min) + min);
                float skyRed = v;
                float skyGreen = v;
                float skyBlue = skyBase;

                if (worldRenderer.getDarkenWorldAmount(tickDelta) > 0.0F) {
                    final float skyDarkness = worldRenderer.getDarkenWorldAmount(tickDelta);
                    skyRed = skyRed * (1.0F - skyDarkness) + skyRed * 0.7F * skyDarkness;
                    skyGreen = skyGreen * (1.0F - skyDarkness) + skyGreen * 0.6F * skyDarkness;
                    skyBlue = skyBlue * (1.0F - skyDarkness) + skyBlue * 0.6F * skyDarkness;
                }

                for (int blockIndex = 0; blockIndex < 16; ++blockIndex) {
                    float blockFactor = 1f;
                    if (!blockAmbient) {
                        blockFactor = 1f - blockIndex / 15f;
                        blockFactor = 1 - blockFactor * blockFactor * blockFactor * blockFactor;
                    }

                    final float blockBase = blockFactor * dim.brightness(blockIndex) * (prevFlicker * 0.1F + 1.5F);
                    min = 0.4f * blockFactor;
                    final float blockGreen = blockBase * ((blockBase * (1 - min) + min) * (1 - min) + min);
                    final float blockBlue = blockBase * (blockBase * blockBase * (1 - min) + min);

                    float red = skyRed + blockBase;
                    float green = skyGreen + blockGreen;
                    float blue = skyBlue + blockBlue;

                    final float f = Math.max(skyFactor, blockFactor);
                    min = 0.03f * f;
                    red = red * (0.99F - min) + min;
                    green = green * (0.99F - min) + min;
                    blue = blue * (0.99F - min) + min;

                    //the end
                    if (ClientLevel.dimension() == Level.END) {
                        red = skyFactor * 0.22F + blockBase * 0.75f;
                        green = skyFactor * 0.28F + blockGreen * 0.75f;
                        blue = skyFactor * 0.25F + blockBlue * 0.75f;
                    }

                    if (red > 1.0F) {
                        red = 1.0F;
                    }

                    if (green > 1.0F) {
                        green = 1.0F;
                    }

                    if (blue > 1.0F) {
                        blue = 1.0F;
                    }

                    final float gamma = (float) client.options.gamma * f;
                    float invRed = 1.0F - red;
                    float invGreen = 1.0F - green;
                    float invBlue = 1.0F - blue;
                    invRed = 1.0F - invRed * invRed * invRed * invRed;
                    invGreen = 1.0F - invGreen * invGreen * invGreen * invGreen;
                    invBlue = 1.0F - invBlue * invBlue * invBlue * invBlue;
                    red = red * (1.0F - gamma) + invRed * gamma;
                    green = green * (1.0F - gamma) + invGreen * gamma;
                    blue = blue * (1.0F - gamma) + invBlue * gamma;

                    min = Math.max(0.03f * f, (float) DarknessDraft.Config.client_minimumLight);
                    red = red * (0.99F - min) + min;
                    green = green * (0.99F - min) + min;
                    blue = blue * (0.99F - min) + min;

                    if (red > 1.0F) {
                        red = 1.0F;
                    }

                    if (green > 1.0F) {
                        green = 1.0F;
                    }

                    if (blue > 1.0F) {
                        blue = 1.0F;
                    }

                    if (red < 0.0F) {
                        red = 0.0F;
                    }

                    if (green < 0.0F) {
                        green = 0.0F;
                    }

                    if (blue < 0.0F) {
                        blue = 0.0F;
                    }

                    LUMINANCE[blockIndex][skyIndex] = luminance(red, green, blue);
                }
            }
        }
    }
}
